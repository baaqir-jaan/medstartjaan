...
def fetch_physician_data(api_url, physician_name, state=None):
    try:
        logger.info(f"Searching for: {physician_name} in state: {state}")
        offset = 0
        page_size = 5000
        timeout = 30
        ...
        for r in data:
            if (r["Rndrng_Prvdr_Last_Org_Name"].lower() == last_name.lower() and 
                r["Rndrng_Prvdr_First_Name"].lower() == first_name.lower()):
                if not state or r.get("Rndrng_Prvdr_State_Abrvtn", "").upper() == state.upper():
